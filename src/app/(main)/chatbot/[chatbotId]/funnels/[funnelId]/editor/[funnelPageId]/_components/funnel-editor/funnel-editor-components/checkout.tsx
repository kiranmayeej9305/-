...
import { getFunnel, getchatbotDetails } from '@/lib/queries'
...
const Checkout = (props: Props) => {
  const { dispatch, state, chatbotId, funnelId, pageDetails } = useEditor()
  const router = useRouter()
  const [clientSecret, setClientSecret] = useState('')
  const [livePrices, setLivePrices] = useState([])
  const [chatbotConnectAccId, setChatbotConnectAccId] = useState('')
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (chatbotDetails) {
        ...
        setChatbotConnectAccId(chatbotDetails.connectAccountId)
      }
    }
    fetchData()
  }, [chatbotId])
  ...
  useEffect(() => {
    if (livePrices.length && chatbotId && chatbotConnectAccId) {
  ...
  }, [livePrices, chatbotId, chatbotConnectAccId])
    ...
    >
      ...
      <div className="border-none transition-all w-full">
        <div className="flex flex-col gap-4 w-full">
          {options.clientSecret && chatbotConnectAccId && (
            <div className="text-white">
              <EmbeddedCheckoutProvider
                stripe={getStripe(chatbotConnectAccId)}
                options={options}
              >
                <EmbeddedCheckout />
              </EmbeddedCheckoutProvider>
